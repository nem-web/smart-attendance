...
const getPillStyle = (color) => {
  switch (color) {
    case "green": return { backgroundColor: "var(--emerald-500)", color: "var(--text-on-primary)" };
    case "amber": return { backgroundColor: "var(--amber-500)", color: "var(--text-on-primary)" };
    case "red": return { backgroundColor: "var(--rose-500)", color: "var(--text-on-primary)" };
    default: return { backgroundColor: "color-mix(in srgb, var(--text-body) 25%, transparent)", color: "var(--text-body)" };
  }
};

const getBarFillStyle = (color) => {
  switch (color) {
    case "green": return { backgroundColor: "var(--emerald-500)" };
    case "amber": return { backgroundColor: "var(--amber-500)" };
    case "red": return { backgroundColor: "var(--rose-500)" };
    default: return { backgroundColor: "var(--text-body)" };
  }
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="px-4 py-2 bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-main)] rounded-lg hover:bg-[var(--bg-hover)] font-medium flex items-center gap-2 transition cursor-pointer">
            <Download size={18} />
            Export list
          </button>
          <button
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          <div className="bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-color)] shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <div className="relative w-full md:w-64">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg text-sm focus:ring-2 focus:ring-[var(--primary)] outline-none"
              />
            </div>
                ...
                <button 
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition ${
                    selectedFilter === filter
                      ? "text-[var(--primary)]"
                      : "text-[var(--text-body)] opacity-70 hover:opacity-100"
                  }`}
                >
                  {filter}
                </button>
          ...
          <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] shadow-sm overflow-hidden">
                  ...
                  {sortedStudents.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="px-6 py-12 text-center">
                        <div className="text-[var(--text-body)] opacity-70">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-[var(--bg-secondary)] flex items-center justify-center text-[var(--text-body)] font-bold text-sm">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3 w-48">
                            {/* The Pill */}
                            <div className="px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap shadow-sm" style={getPillStyle(color)}>
                              {percentage}% {status}
                            </div>

                            {/* The Bar */}
                            <div className="h-2 flex-1 rounded-full overflow-hidden" style={{ backgroundColor: "color-mix(in srgb, var(--text-body) 12%, transparent)" }}>
                              <div className="h-full rounded-full" style={{ ...getBarFillStyle(color), width: `${percentage}%` }} />
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          {trend > 0 ? (
                            <div className="flex items-center gap-1 text-xs font-semibold" style={{ color: "var(--emerald-500)" }}>
                              <ArrowUpRight size={14} />
                              +{trend}% vs last month
                            </div>
                          ) : trend < 0 ? (
                            <div className="flex items-center gap-1 text-xs font-semibold" style={{ color: "var(--rose-500)" }}>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button className="text-[var(--text-body)] opacity-70 hover:opacity-100 p-1 hover:bg-[var(--bg-hover)] rounded-full transition">
            ...
            {/* Pagination Footer */}
            <div className="px-6 py-4 border-t border-[var(--border-color)] flex items-center justify-between text-xs text-[var(--text-body)] opacity-70">
              <span>Showing {sortedStudents.length} of {verifiedStudents.length} students</span>
              <div className="flex gap-2">
                <button className="px-3 py-1 border border-[var(--border-color)] rounded hover:bg-[var(--bg-hover)] cursor-pointer">Previous</button>
                <button className="px-3 py-1 border border-[var(--border-color)] rounded hover:bg-[var(--bg-hover)] cursor-pointer">Next</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-xl border border-[var(--border-color)] shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ backgroundColor: "color-mix(in srgb, var(--emerald-500) 18%, transparent)", color: "var(--emerald-500)" }}>{"> 90%"}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--text-body)]">Medium attendance</span>
                <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ backgroundColor: "color-mix(in srgb, var(--amber-500) 18%, transparent)", color: "var(--amber-500)" }}>{"75-90%"}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--text-body)]">Low attendance</span>
                <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ backgroundColor: "color-mix(in srgb, var(--rose-500) 18%, transparent)", color: "var(--rose-500)" }}>{"< 75%"}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{backgroundColor: "color-mix(in srgb, var(--primary) 18%, transparent)", color: "var(--primary)"}}>{i+1}</div>
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-xl border border-[var(--border-color)] shadow-sm">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: "color-mix(in srgb, var(--rose-500) 18%, transparent)", color: "var(--rose-500)" }}>{i+1}</div>
                    <span className="text-sm font-medium text-[var(--text-body)]">{s.name}</span>
                  </div>
                  <span className="text-sm font-bold text-[var(--text-main)]">{s.val}</span>
                </div>
